...
void *mymalloc(size_t size) {
    ...
    if (isFound == true) {
        int startingAddress = bestNode->key;
        td = bestNode->pdata;
        // mutate node
        size_t remainingSize = td->val - size;
        // printf("Best Node - Start index: %d Length: %d\n", bestNode->key, bestNode->pdata->val);
        bestNode->pdata->val = size;
        bestNode->pdata->isOccupied = true;
        ...
        if (remainingSize != 0) {
            ...
            int nextAddress = bestNode->key + size;
